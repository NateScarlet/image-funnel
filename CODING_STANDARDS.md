...
- **ID**: 客户端不应尝试解析 ID，格式不保证不固定
- **依赖注入：**　**禁止**可选依赖，调用者必须显式传入空实现；**禁止**自己创建自己的依赖，仅允许在程序最外层入口构建依赖
- **按需构建请求级依赖：**　最外层入口构建每请求的基础设施依赖时，若某个依赖仅服务于特定分支/指令（如某指令独占的全局配置），应仅在对应分支触发时构建并注入，⛔不得在公共请求路径无条件构建。否则会把该分支独有的全局依赖（如必须存在的环境变量或配置文件）扩散到所有请求，导致无关请求因缺失该依赖而整体失败。
...
- **应用层日志**: 所有应用层 Handler 的公开 command 方法必须记录日志，遵循 `will`/`did` 模式：
...
- **最小参数设计与空值处理**: 在传递 Filter 筛选对象、组件属性、Composable 选项或调用 GraphQL Mutation 时，遵循**最小参数设计原则**。若不需要某项筛选条件或参数（如无评级筛选、无标签筛选、无搜索关键字等），**禁止**透传空切片/空数组（如 `label: []`）、空字符串（如 `query: ""`）或零值占位符；而应直接省略或设为 `undefined`。由于 Go 后端对非 `nil` 的空切片（如 `label: []`）具有“要求过滤但选中集合为空”的语义（会导致 0 匹配），必须使用 `optionalArray` 工具函数规避空切片透传。
- **禁止防御性空数组兜底（`?? []`）**: 在读取 Filter 筛选对象（如 `filter.rating` / `filter.label`）时，`null` / `undefined` 代表“**未限制条件 = 匹配全部**”，而 `[]` 代表“**限制条件为空集 = 0 匹配**”。**严禁**盲目使用 `val ?? []` 或 `optionalArray(val) ?? []` 进行防御性兜底，这会将 `undefined`（全匹配）错误扭曲为 `[]`（0 匹配），从而打破核心业务逻辑。
...
- 标注所有参数类型，尽量避免 Any
- 使用项目提供的 `pwsh ./scripts/check-python.ps1` 脚本进行类型检查、单元测试和格式化
