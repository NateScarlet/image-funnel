...
**图片变体 / Image Variant**
图片的派生转码产物，由源图身份（文件名、修改时间、大小）与宽度、格式唯一确定。永不修改源图，变体产物可随时按需重建。前端按展示分辨率请求对应变体（如 256/512/1024/2048/4096 宽度档位）；缩略图只是变体的一个分辨率档位。画质不参与变体身份：各编码器的画质由服务端固定配置决定（AVIF 见 `svtav1CRF`、WebP 见 `webpQuality`），客户端不能指定，因此同一档位不会因画质参数不同而重复编码。
_避免_: thumbnail, cached image（缓存是实现细节而非概念）

**图片访问授权 / Image Access Grant**
取回某张图片内容的凭据：持有它即可取回对应内容，无需登录态；一旦失效即被拒绝。它绑定的是**图片身份**（相对路径 + 修改时间 + 大小）以及请求的变体参数——同路径的文件被替换后身份改变，原授权随之失效。文件变更后需要重新签发。
_避免_: 签名 URL、token（都是载体或实现手段，不是这个概念本身）
...
**复制增强 / Copy Enhancement**
声明 `[copy]` 能力标记的外部钩子，在用户复制图片时被同步调用：脚本自行读取图片元数据并返回应写入剪贴板的内容（单行 JSON 信封），核心应用不含任何特定生成工具的知识。全局至多一个 `[copy]` 钩子（重复配置报错列出冲突 id）；脚本对不适用的图片输出空内容即表示放弃，前端降级为复制文件本体。首个示例为 ComfyUI 工作流复制（输出目录调整为图片当前所在目录）。ComfyUI 元数据中 prompt 一侧的 `filename_prefix` 恒为已求值的静态快照（空值变量会残留连续分隔符），`%…%` 模板语法只存在于 workflow 一侧的 widgets_values 中。
