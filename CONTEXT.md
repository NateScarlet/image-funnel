...
声明 `[copy]` 能力标记的外部钩子，在用户复制图片时被同步调用：脚本自行读取图片元数据并返回应写入剪贴板的内容（单行 JSON 信封），核心应用不含任何特定生成工具的知识。全局至多一个 `[copy]` 钩子（重复配置报错列出冲突 id）；脚本对不适用的图片输出空内容即表示放弃，前端降级为复制文件本体。首个示例为 ComfyUI 工作流复制（输出目录调整为图片当前所在目录）。
