...
- 优先使用 `scripts/build.ps1`，避免直接运行底层命令（如`go build`，`pnpm build`）
...
- **js:** 禁止使用 eslint 注释来禁用规则，如果不知道怎么处理应询问用户而不是忽略项目要求
...
- **go:** 新增功能的同时，添加对应的单元测试
  　- 测试文件名应和逻辑所在的文件对应
- **go:** 对需清理资源的构建函数，返回清理函数作为第二个返回值
- **vue:** composable 不要接收非响应对象，参数无特殊理由的话都应该用 MaybeRefOrGetter
- **css:** 优先使用tailwind配置值，尽量避免使用任意值语法，禁止使用像素指定尺寸
- **vue:** 组件名称应该至少两个单词
