...
def process_remove_prompt(
    text: str,
    prompt_to_remove: str,
    start_marker: str,
    end_marker: str,
    raw: bool,
    no_skip: bool,
    ...
    """
    移除指定提示词，返回修改后的文本。若找不到且不 skip，返回 None。
    """
    start_idx = text.find(start_marker)
    end_idx = text.find(end_marker)

    has_marker = start_idx != -1 and end_idx != -1 and start_idx < end_idx
    target_text = text[start_idx + len(start_marker) : end_idx] if has_marker else text

    def contains_prompt(area: str) -> bool:
        target_lower = prompt_to_remove.strip().lower()
        if raw:
            return target_lower in area.lower()
        for line in area.splitlines():
            if line.strip().startswith("//"):
    ...
    if not contains_prompt(target_text):
        if not no_skip:
            _LOGGER.info(f"Prompt '{prompt_to_remove}' not found, skipping.")
            return None
        return text
    ...
    if raw:
        new_target = target_text.replace(prompt_to_remove, "")
    else:
        target_lower = prompt_to_remove.strip().lower()
