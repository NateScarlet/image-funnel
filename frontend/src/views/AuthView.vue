...
<template>
  ...
  >
    ...
    >
      ...
      <!-- 通过可信 IP 访问且未绑定设备 -->
      <div
        v-else-if="isTrustedIP && !isTrustedDevice"
      ...
      >
        ...
        >
          ...
          <h2 class="mb-2 text-xl font-semibold text-success-100">
            可信 IP 访问已启用
          </h2>
          <p class="text-sm">
            您当前正通过可信 IP 访问，无需登录即可使用应用。
          </p>
        </div>
        ...
        <div class="space-y-4 text-left">
          ...
          <ul
            class="list-disc space-y-2 pl-5 text-primary-300 text-xs leading-relaxed"
          >
            <li>
              将对应环境的 IP 地址配置到服务端的
              <code
            ...
            <li v-else class="text-primary-400">
              当前浏览器不支持设备认证（WebAuthn）。若想绑定设备，请通过 HTTPS
              或使用支持 WebAuthn 的现代浏览器访问。
            </li>
          </ul>
        </div>
        ...
        <div
          ...
          class="space-y-4 pt-4 border-t border-primary-700/30"
        >
