    ...
    <div class="bg-slate-800 rounded-lg p-6 w-full max-w-md">
      ...
      <div class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-300 mb-2">
            目标保留数量
            <span class="text-slate-400 ml-2 text-xs">({{ kept }} / {{ targetKeep }})</span>
          </label>
          <input v-model.number="targetKeep" type="number" min="1"
      ...
      <div class="mt-6 flex justify-end gap-3">
        <button class="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm transition-colors"
          @click="$emit('close')">
          取消
        </button>
        <button class="px-4 py-2 bg-secondary-600 hover:bg-secondary-700 rounded-lg text-sm transition-colors"
